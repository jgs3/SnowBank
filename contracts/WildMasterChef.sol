...
/// @title Wild MasterChef
// MasterChef is the master of WILD token. He can make WILD and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once WILD is sufficiently
// distributed and the community can show to govern itself.
//
// Have fun reading it. Hopefully it's bug-free. God bless.
contract WildMasterChef is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of WILDs
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accWildPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws tokens to a pool. Here's what happens:
        //   1. The pool's `accWildPerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. WILDs to distribute per block.
        uint256 totalStaked; // Amount of tokens staked in given pool
        uint256 lastRewardTime; // Last timestamp WILDs distribution occurs.
        uint256 accWildPerShare; // Accumulated WILDs per share, times 1e18. See below.
        uint16 depositFeeBP; // Deposit fee in basis points
        bool withDepositLockDiscount; //Allows to lock funds for deposit discound
    }

    // The WILD TOKEN
    WildToken public immutable wild;
    // Dev address.
    address public devaddr;
    // Dev fee percentage.
    uint256 public devFee = 100;
    // WILD tokens created per second.
    uint256 public wildPerSecond;
    // Deposit Fee address
    ...
    // The block number when WILD mining starts.
    ...
    event UpdateEmissionRate(address indexed user, uint256 wildPerSecond);
    ...
    constructor(
        ...
        address _devaddr,
        address _feeAddress,
        uint256 _wildPerSecond,
        uint256 _startTime
    ) {
        require(_devaddr != address(0), "zero address");
        require(_feeAddress != address(0), "zero address");
        require(_wildPerSecond <= MAX_EMISSION_RATE, "max emission rate exceeded");
        _wild.balanceOf(address(this)); //safety check

        wild = _wild;
        devaddr = _devaddr;
        feeAddress = _feeAddress;
        wildPerSecond = _wildPerSecond;
    ...
    function add(
        ...
        emit PoolAdded(
            ...
            _stakeToken,
            _depositFeeBP,
            _withDepositLockDiscount
        );
    }

    /// @notice Update the given pool's WILD allocation point and deposit fee. Can only be called by the owner.
    /// @param _pid pool id
    /// @param _allocPoint new allocation
    ...
    /// @notice View function to see pending WILDs on frontend.
    /// @param _pid pool id
    ...
    function pendingTokens(uint256 _pid, address _user) external view returns (uint256) {
        ...
        uint256 accWildPerShare = pool.accWildPerShare;
        uint256 stakeSupply = pool.totalStaked;
        if (block.timestamp > pool.lastRewardTime && stakeSupply != 0) {
            uint256 multiplier = getMultiplier(pool.lastRewardTime, block.timestamp);
            uint256 totalWild = multiplier.mul(wildPerSecond).mul(pool.allocPoint).div(
                totalAllocPoint
            );
            uint256 forDevs = totalWild.mul(devFee).div(1000);
            uint256 wildReward = totalWild.sub(forDevs);
            accWildPerShare = accWildPerShare.add(wildReward.mul(1e18).div(stakeSupply));
        }
        return user.amount.mul(accWildPerShare).div(1e18).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardTime, block.timestamp);
        uint256 totalWild = multiplier.mul(wildPerSecond).mul(pool.allocPoint).div(totalAllocPoint);
        if (totalWild == 0) return;

        uint256 forDevs = totalWild.mul(devFee).div(1000);
        uint256 wildReward = totalWild.sub(forDevs);
        wild.mint(devaddr, forDevs);
        wild.mint(address(this), wildReward);
        pool.accWildPerShare = pool.accWildPerShare.add(wildReward.mul(1e18).div(stakeSupply));
        pool.lastRewardTime = block.timestamp;
        emit PoolUpdated(_pid, pool.lastRewardTime, stakeSupply, pool.accWildPerShare);
    }
    ...
    function deposit(
        ...
        updatePool(_pid);
        if (user.amount > 0) {
            pending = user.amount.mul(pool.accWildPerShare).div(1e18).sub(user.rewardDebt);
            if (pending > 0) {
                safeWildTransfer(msg.sender, pending);
            }
        }
        if (_amount > 0) {
        ...
        user.rewardDebt = user.amount.mul(pool.accWildPerShare).div(1e18);
    ...
    function depositFor(
        ...
        if (user.amount > 0) {
            pending = user.amount.mul(pool.accWildPerShare).div(1e18).sub(user.rewardDebt);
            if (pending > 0) {
                safeWildTransfer(_recipient, pending);
            }
        }
        if (_amount > 0) {
            // Prefetch balance to account for transfer fees
            uint256 preStakeBalance = pool.stakeToken.balanceOf(address(this));
            pool.stakeToken.safeTransferFrom(address(_sender), address(this), _amount);
            finalDepositAmount = pool.stakeToken.balanceOf(address(this)) - preStakeBalance;

            if (pool.depositFeeBP > 0 && !partners[_recipient]) {
                uint256 depositFee = finalDepositAmount.mul(pool.depositFeeBP).div(10000);
                if (userInfo[_pid][_recipient].unlockTime > block.timestamp) {
                    userInfo[_pid][_recipient].unlockTime = uint64(
                        block.timestamp +
                            calculateNewUnlockTimeForUser(_recipient, _pid, 0, _amount)
                    );
                }
                pool.stakeToken.safeTransfer(feeAddress, depositFee);
                finalDepositAmount = finalDepositAmount.sub(depositFee);
        ...
        user.rewardDebt = user.amount.mul(pool.accWildPerShare).div(1e18);
        emit Deposit(_recipient, _pid, finalDepositAmount);
    }
    ...
    function withdraw(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        updatePool(_pid);
        uint256 pending = user.amount.mul(pool.accWildPerShare).div(1e18).sub(user.rewardDebt);
        if (pending > 0) {
            safeWildTransfer(msg.sender, pending);
        }
        if (_amount > 0) {
    ...
    /// @dev Safe WILD transfer function, just in case if rounding error causes pool to not have enough WILDs.
    function safeWildTransfer(address _to, uint256 _amount) internal {
        uint256 wildBal = wild.balanceOf(address(this));
        bool transferSuccess = false;
        if (_amount > wildBal) {
            transferSuccess = wild.transfer(_to, wildBal);
        } else {
            transferSuccess = wild.transfer(_to, _amount);
        }
        require(transferSuccess, "safeWildTransfer: transfer failed");
    }
    ...
    /// @notice udpate WILD emission rate
    /// @param _wildPerSecond new emission rate. Must be less than 10 WILD/second
    function updateEmissionRate(uint256 _wildPerSecond) external onlyOwner {
        massUpdatePools();
        _updateEmissionRate(_wildPerSecond);
    }

    function _updateEmissionRate(uint256 _wildPerSecond) internal {
        require(_wildPerSecond <= MAX_EMISSION_RATE, "Updated emissions are more than maximum rate");
        wildPerSecond = _wildPerSecond;
        emit UpdateEmissionRate(msg.sender, _wildPerSecond);
    }

    /// @notice sets devs fee
    /// @param _newDevFee new WILD fee in base points. Must be less than MAX_DEV_FEE
